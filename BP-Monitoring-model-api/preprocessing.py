...
class EnsembleModel(BaseEstimator, RegressorMixin):
    def __init__(self, rf_model, ab_model):
        self.rf_model = rf_model
        self.ab_model = ab_model

    def fit(self, X, y):
        # In this case, models are pre-trained, so fitting is not needed.
        # If models weren't pre-trained, you would fit them here.
        return self

    def predict(self, X):
        # Get predictions from all models
        rf_preds = self.rf_model.predict(X)
        ab_preds = self.ab_model.predict(X)
        
        # Average the predictions
        ensemble_preds = (rf_preds * 0.6 + ab_preds * 0.4)
